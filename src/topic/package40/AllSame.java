...
public class AllSame {
    ...
    // right 可能性 : N
    // midV 可能性 : arr中的最大值！
    public static int process(int[] arr, int left, int midVal, int right) {
        for (; left >= 0 && arr[left] == midVal; left--) ;
        for (; right < arr.length && arr[right] == midVal; right++) ;
        if (left == -1 && right == arr.length) return 0;
        int lcount = Integer.MAX_VALUE, rcount = Integer.MAX_VALUE;
        if (left >= 0) {
            lcount = process(arr, left - 1, arr[left], right);
        }
        if (right < arr.length) {
            rcount = process(arr, left, arr[right], right + 1);
        }
